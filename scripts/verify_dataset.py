...
VALID_CATEGORIES = {"leave", "payroll", "benefits", "onboarding", "offboarding", "contract", "workplace_issue", "policy", "training", "personal_data", "other"}
...
def main() -> None:
    ...
    assert {record["gold_action"] for record in records} <= {"direct_answer", "human_review"}
    assert Counter(record["gold_ideal_route"] for record in records) == {"qwen": 25, "rules": 20, "human": 15}
    ...
    print("Dataset verified: 60 records; 15 dev / 45 test; labels and IDs valid.")
